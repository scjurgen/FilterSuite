...
{ 
    ...
    alpha2 = 0.5;
    int ap = 0;
    progname = (argv[ap] != NULL) ? argv[ap++] : (char*)"???";
    until (argv[ap] == NULL)
    { 
        char *s = argv[ap++];
        if (seq(s,"-a"))
        { 
            ...
            unless (alpha2 > alpha1)
                giveup("Invalid values for -a: a1 must be .lt. a2");
        }
        else if (gtype == unknown && seq(s,"-l")) 
            gtype = locus;
        else if (gtype == unknown && (seq(s,"-i") || seq(s,"-s")))
        {
            gtype = (s[1] == 's') ? step : impulse;
            nirsteps = getiarg(argv[ap++]);
            if (nirsteps < 0 || nirsteps > MAXSTEPS) 
                giveup("Invalid value for num steps: limits are 0 .. %d", MAXSTEPS);
        }
        else if (seq(s,"-log"))
        { 
            logmin = getfarg(argv[ap++]);
            if (logmin >= 0.0)
                giveup("Invalid value for log min (%s): must be .lt. 0", argv[ap-1]);
            logopt = true;
        }
        else if (seq(s,"-d"))
            dflag = true;
...
{ 
    ...
    { 
        ...
        { 
            ...
            unless (ok)
                fprintf(stderr, "genplot: warning: step length %g in locus plot\n", stepl);
            break;
        }
...
{ 
    ...
    int i;
    for (i=0; i <= nzeros; i++)
        topcos[i] = complex(xcoeffs[i]);
    for (i=0; i <= npoles; i++)
        botcos[i] = complex(-ycoeffs[i]);
    double maxmag = 0.0;
    for (i = 0; i <= nsteps; i++)
    { 
        ...
        if (mag > maxmag) 
            maxmag = mag;
        if (dflag) 
            fr[i] = fr[i] * expj(0.5 * nzeros * theta);
    }
    if (maxmag > 0.0)
    { 
        for (i=0; i <= nsteps; i++) 
            fr[i] /= maxmag;  /* scale so max mag is 1.0 */
    }
}
...
{ 
    ...
    { 
        ...
        yv[npoles] = 0.0;
        for (j=0; j <= nzeros; j++) 
            yv[npoles] += (xcoeffs[j] * xv[j]);
        for (j=0; j < npoles; j++) 
            yv[npoles] += (ycoeffs[j] * yv[j]);
        ir[i] = yv[npoles];
...
{ 
    ...
    { 
        ...
        for (i = 0; i <= 10; i++)
            draw_ytick(im, 0.0, 0.1*i, red, -TICKLEN, fmt, (1.0 - 0.1*i) * logmin);
    }
    else
    { 
        for (i = 0; i <= 10; i++) 
            draw_ytick(im, 0.0, 0.1*i, red, -TICKLEN, "%3.1f", 0.1*i);
    }
    draw_ytick(im, 1.0, 0.0, blue, TICKLEN, "-pi");
    draw_ytick(im, 1.0, 0.5, blue, TICKLEN, "0");
    draw_ytick(im, 1.0, 1.0, blue, TICKLEN, "+pi");
    for (i = 1; i <= 9; i++)
        draw_ytick(im, 1.0, 0.1*i, blue, TICKLEN, "", 0.2*(i-5));
...
{ 
    ...
    gdImageLine(im, xmap_pm(1.0), ymap_pm(0.0), xmap_pm(1.0), ymap_pm(1.0), black); /* right Y axis */
    for (i=0; i <= nsteps; i++)
        ymvals[i] = 0.5 * (ir[i] + 1.0);
    /* ymvals in range 0 .. 1 */
    pgraph(im, ymvals, nsteps, red, false);
    for (i=0; i <= 10; i++)
        draw_ytick(im, 0.0, 0.1*i, red, -TICKLEN, "%3.1f", 0.2*(i-5));
    for (i=0; i <= 10; i++)
    { 
    ...
    { 
        ...
        { 
            ...
            { 
                ...
                gdImageLine(im, xmap_pm(x0), ymap_pm(y0), xmap_pm(x1), ymap_pm(y1), col);
        }
    }
}
...
{ 
    ...
    if (tlen < 0)
        gdImageString(im, gdFontSmall, ix + 2*tlen - swid, iy - shgt/2, (unsigned char *)str, col);
    if (tlen > 0)
        gdImageString(im, gdFontSmall, ix + 2*tlen, iy - shgt/2, (unsigned char *)str, col);
}
