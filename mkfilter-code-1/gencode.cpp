...
{ 
    ...
    { 
        int n = 0;
        while (n < npoles && ycoeffs[n] == 0.0 && xcoeffs[n] == 1.0) 
            n++;
        if (n >= npoles) 
            mvavg = true;
    }
...
{ 
    ...
    { 
        ...
        for (int i=0; i <= nzeros; i++)
            printf("\t%15.8e", xcoeffs[i]);
        for (int i=0; i <= npoles; i++) 
            printf("\t%15.8e", ycoeffs[i]);
        putchar('\n');
    }
    else
    { 
...
{ 
    ...
    printf("  { for (;;)\n");
    printf("      { "); 
    pr_shiftdown("xv", nzeros); putchar('\n');
    printf("        xv[%d] = `next input value' / GAIN;\n", nzeros);
    printf("        "); 
    pr_shiftdown("yv", npoles); putchar('\n');
    printf("        yv[%d] =", npoles);
    pr_xpart("xv"); 
    pr_ypart("yv"); 
    printf(";\n");
...
{ 
    ...
    { 
        double f = fmod(x, 1.0);
        char *fmt = (f < EPS || f > 1.0-EPS) ? (char*)"%g" : (char*)"%14.10f";
        printf(fmt, x); 
        printf(" * ");
    }
    nxfacs++;
