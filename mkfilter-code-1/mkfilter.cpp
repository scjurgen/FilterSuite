...
{ 
    unless (options & opt_p) 
        polemask = ~0; /* use all poles */
    unless (options & (opt_bp | opt_bs)) 
...
{ 
    printf("raw alpha1    = %14.20f\n", raw_alpha1);
    printf("raw alpha2    = %14.20f\n", raw_alpha2);
    unless (options & (opt_re | opt_w | opt_z))
    {
        printf("warped alpha1 = %14.20f\n", warped_alpha1);
        printf("warped alpha2 = %14.20f\n", warped_alpha2);
    }
    printgain("dc    ", dc_gain);
    printgain("centre", fc_gain);
    printgain("hf    ", hf_gain);
    putchar('\n');
    unless (options & opt_re) 
        printrat_s();
    printrat_z();
    printrecurrence();
}
...
{ 
    ...
    if (r > EPS) 
        printf("   phase = %14.20f pi", atan2(gain) / PI);
    putchar('\n');
}
...
{ 
    ...
    { 
        ...
        char *fmt = (f < EPS || f > 1.0-EPS) ? (char*)"%3g" : (char *)"%14.20f";
        putchar('('); 
        printf(fmt, x);
        printf(" * x[n-%2d])\n", zplane.numzeros-i);
    }
...
{ 
    printf("%14.20f + j %14.20f", z.re, z.im);
}
